		...
		<PageItem>
			<header className='flex items-center justify-between mb-[1.667vw] pb-[0.903vw] border-b-[0.069vw] border-b-[#F0F3F8]'>
				<div className='hidden sm:flex gap-[0.833vw] items-center'>
					...
					<p className='text-[2.188vw] xl:text-[1.389vw] font-medium'>
						Time Off
					</p>
				</div>
				<div className='flex w-full flex-col sm:flex-row items-end sm:items-center gap-[2.083vw] mb-[3.5vw] sm:mb-0'>
					<p className='text-[2.031vw] lg:text-[1.172vw] xl:text-[0.972vw] font-medium'>
					...
					<button className='p-[0.556vw] border-[0.069vw] border-black rounded-[0.625vw]'>
						<p className='text-[1.719vw] lg:text-[0.972vw] font-medium'>
							Add Time Off Policy
						</p>
					</button>
				</div>
			...
			<div className='sm:hidden'>
				<Swiper slidesPerView={1.2} spaceBetween={20}>
					<SectionItemPlusBorder>
						<div className='flex justify-center'>
							{timeList.map((item, i) => (
								<SwiperSlide key={i}>
									<div className='flex flex-col items-center gap-[0.278vw] w-full py-[5vw] font-semibold bg-[#F0F3F8] rounded-[0.556vw]'>
										<h2 className='text-[4.375vw]'>{item.title}</h2>
										<TextPlusImg
											textSize='4vw'
											imgSize='size-[4vw]'
											img={item.stats.img}
										...
										<p className='text-[3vw]'>{item.possibility}</p>
									</div>
								</SwiperSlide>
							))}
						</div>
				...
				<TextPlusImg className='my-[2vw]' img='/svg/full-time.svg'>
					Upcoming Time Off
				</TextPlusImg>
			</div>
			{/* MEDICAL  */}
			<SectionItemPlusBorder>
				<div className='flex items-center gap-[0.972vw]'>
					<ImageCustom
						size='size-[4vw] sm:size-[2.083vw]'
						src='/svg/medical.svg'
						alt='medical'
					/>
					<div className='flex flex-col items-start text-[2vw] sm:text-[1.172vw] xl:text-[0.972vw] font-medium'>
						<span>Jan 27</span>
						<div className='flex gap-[0.278vw] items-center'>
							<div className='size-[0.694vw] bg-black rounded-full'></div>
			...
			<SectionItemPlusBorder>
				<div className='flex items-center gap-[0.972vw]'>
					<ImageCustom
						...
						alt='big-many'
					/>
					<div className='flex flex-col text-[2vw] sm:text-[1.172vw] xl:text-[0.972vw] font-medium'>
						<p>Jul 4</p>
						<p>Independence Day</p>
					</div>
				</div>
			</SectionItemPlusBorder>
			<TextPlusImg className='mb-[1.111vw]' img='/svg/history.svg'>
				History
			</TextPlusImg>
			{/* FILTER SECTION  */}
			<div>
				...
				<div className='hidden sm:block'>
					...
					<div className='pl-[0.556vw] mt-[0.972vw] pb-[6.319vw]'>
						{tableValue.map((value, i) => (
							<div
								className='grid grid-cols-custom-5-lg xl:grid-cols-custom-5 py-[0.556vw] border-b-[0.139vw] border-b-[#F0F3F8] lg:hover:bg-[#DAE6F2] duration-300 ease-in-out cursor-pointer'
								key={i}
							>
